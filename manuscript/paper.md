...
![Modules integrated into the Table and Results modules. The match module enables matching of multiple data sets, exemplified here by retention time matching between two signals collected on different systems. The formula module enables the creation of Excel-style formulas for dynamic reporting. The categories module enables the rapid labelling of datapoints using a user-specified category dictionary. In the top-right, “C1” and “C2” refer to different columns or instruments.\label{fig:modules}](Fig-Modules_08-23-2026.png)
...
The above example demonstrates just one use of the `ChromaQuant` package. There are countless possible applications of `ChromaQuant` to complement many different chromatographic techniques. For example, the analysis of evaporative light scattering detector data (ELSD) is common in HPLC systems, and can be quantified using a nonlinear formula (\autoref{eq:ELSD}). 

\begin{equation}\label{eq:ELSD}
A=aC^b
\end{equation}

where $A$ is the area under a chromatogram peak, $C$ is the concentration, and $a$ and $b$ are constants. `ChromaQuant` facilitates the seamless assignment of factors $a$ and $b$ using the `Match` module and enables dynamic reporting by defining nonlinear formulas with the `Formula` module. Another extension of `ChromaQuant` is the analysis of data collected by diode array detectors (DADs), which are commonly found in HPLC and used in compound identification. Combined with existing UV spectral library comparison tools, `ChromaQuant` enables the comparison of component tables to data collected on parallel detectors using the `Match` module. As with the ELSD example, `ChromaQuant` facilitates rapid RF assignment and dynamic reporting. 
